...
_datasetFeaturesFiles = {"miniimagenet": "../checkpoints/miniImagenet/WideResNet28_10_S2M2_R/last/output.plk",
                         "cub": "../checkpoints/CUB/WideResNet28_10_S2M2_R/last/output.plk",
                         "cub_base": "../checkpoints/CUB/WideResNet28_10_S2M2_R/last/base_output.plk",
                         "cub_val": "../checkpoints/CUB/WideResNet28_10_S2M2_R/last/val_output.plk",
                         "cifar": "../checkpoints/cifar/WideResNet28_10_S2M2_R/last/test_output.plk",
                         "cifar_base": "../checkpoints/cifar/WideResNet28_10_S2M2_R/last/base_output.plk",
                         "cifar_val": "../checkpoints/cifar/WideResNet28_10_S2M2_R/last/val_output.plk",
                         "cross": "../checkpoints/cross/WideResNet28_10_S2M2_R/last/output.plk",
                         "miniimagenet_base": "../checkpoints/miniImagenet/WideResNet28_10_S2M2_R/last/base_output.plk",
                         "miniimagenet_val": "../checkpoints/miniImagenet/WideResNet28_10_S2M2_R/last/val_output.plk"
                         }
_cacheDir = "./cache"
_maxRuns = 1
_min_examples = -1
...
def GenerateRun_pro(iRun, cfg, regenRState=False, generate=True):
    global _randStates, data, _min_examples
    if not regenRState:
        np.random.set_state(_randStates[iRun])
...
def setRandomStates(cfg,_MaxRuns):
    global _randStates,  _rsCfg, _maxRuns
    _maxRuns = _MaxRuns
    if _rsCfg == cfg:
        return
...
def GenerateRunSet(start=None, end=None, cfg=None):
    ...
    if cfg is None:
        cfg = {"shot": 5, "ways": 5, "queries": 15}

    setRandomStates(cfg, end)
    print("generating task from {} to {}".format(start, end))
    ...
    return dataset
def GenerateRunSet_and_pro(start=None, end=None, cfg=None):
    global dataset, _maxRuns
    if start is None:
        start = 0
    if end is None:
    ...
    dataset = torch.zeros(
        (end-start, cfg['ways'], cfg['shot']+cfg['queries'], data.shape[2]))
    classes = [0 for i in range(end)]
    for iRun in range(end-start):
        dataset[iRun], classes[iRun] = GenerateRun_pro(start+iRun, cfg)

    return dataset, classes

def chang_train_data(train_data):
    global data
    data = train_data
