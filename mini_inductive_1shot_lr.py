...
# ---- data loading
    ...
    log = log_creater('./log/mini_1/inductive/lr')
    ...
    for run in range(train_feature_runs.size(0)):
        ...
        if run % 1000 == 0 and run > 0:
            ...
            for runs in range(20):
                torch.cuda.empty_cache()
                shot_features = ndatas[runs][:n_shot * n_ways]
                pro = torch.zeros(5, 640).cuda()
                for i in range(5):
                    pro[i] = shot_features[i]
                logits = euclidean_metric(pro, train_pro)
                new_feature_train = torch.arange(384000.0).reshape(600, 640).cuda()
                for j in range(5):
                    torch.cuda.empty_cache()
                    ...
                    decoder.eval()
                    noise = encoder(class_train, proty)
                    new_feature = decoder(pro[j].repeat(600, 1), noise)
                    new_feature_train = torch.cat((new_feature_train, new_feature), dim=0)
                new_feature_train = torch.cat((new_feature_train[600:], shot_features), dim=0)
            ...
            if acc > best_acc:
                ...
                best_acc = acc
                last_file_name = os.getcwd() + "/model_weights/" + 'mini_inductive_lr' + '_' \
                                 + str(n_shot) + '_shot_' \
                                 + str(run) + '_run_' \
